...
namespace sc2 {
    ...
    inline Point2D from_proto(const proto::Point2D& input)
    {
    ...
    inline UnitCommand from_proto(const proto::ActionRawUnitCommand& unit_command)
    {
        UnitCommand result;
    ...
    inline ActionError from_proto(const proto::ActionError& x)
    {
    ...
    inline Point3D from_proto(const proto::Point& x)
    {
    ...
    inline auto from_proto(const proto::UnitOrder& x)
    {
        assert(x.ability_id());
        return UnitOrder{
            .ability_id = x.ability_id(),
    ...
    inline auto from_proto(const proto::PassengerUnit& x)
    {
    ...
    inline auto from_proto(const proto::RallyTarget& x)
    {
        return RallyTarget{
    ...
    inline Unit from_proto(const proto::Unit& x)
    {
         ...
         > convert_observation(const proto::ResponseObservation& response_observation)
    {
        ...
        return std::make_tuple( std::vector<Unit>{units.begin(), units.end()}
                              , std::vector<UnitCommand>{ actions_raw.begin(), actions_raw.end()}
                              , std::vector<ActionError>{ errors.begin(), errors.end()}
                              );
    }
}
